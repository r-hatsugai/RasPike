...
void DirectionClass::Direction_r_update(float leftdistance,float rightdistance)
{
...
void DirectionClass::Direction_l_update(float leftdistance,float rightdistance)
{
    //(360 / (2 * 円周率 * 車体トレッド幅)) * (右進行距離-左進行距離)
    Direction += (360.0 / (2.0 * 3.14 * 125)) * (rightdistance - leftdistance);
    // printf("Direction%f\n", Direction);
}
